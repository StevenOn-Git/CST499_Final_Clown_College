...
<body>
		...
		if(isset($_POST['submit'])){
			...
			try{
				...
				if ($rowCount1 > 0){
					...
					$sectionID = parse_url($url, PHP_URL_QUERY);
					if($sectionID >0){
						$result = $sqlConnect->post_RegRoster($pdo,$sectionID,$row1['id']);
						if($result==1){
							echo "Success!";
	...
	<div class="container-fluid" style="text-align: center; padding-top: 25px;">
		<h2>Do you want to register for this course?</h2>
	</div>
	<hr size="1">
	<!-- Login Form -->
	<form method="post" action="register.php?<?php $url= $_SERVER['REQUEST_URI']; echo parse_url($url, PHP_URL_QUERY);?>">
		<div class="container">
			<div class="row">
				<div class="col-6">
			...
			<hr size="1">			
			<button name="submit" type="submit" class="btn btn-outline-dark btn-lg form-control">Register</button>
			<hr size="1">			
